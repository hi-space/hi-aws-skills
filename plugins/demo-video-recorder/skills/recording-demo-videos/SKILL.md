...
Decide the message first, then record the real UI in wall-clock time with Playwright `recordVideo` while an in-page CSS camera pans and zooms; then burn the story's captions, render the intro and summary cards, and cut a reel with ffmpeg from a JSON manifest. The video argues one thing (or, for a feature tour, shows what each feature does); chapters are its answers, captions are the steps, the frame is the evidence for each step. Captions never describe what is on screen, and they are written as plain 합니다체 sentences without em dashes, middle dots, arrows, speed labels or notes about the recording. Nothing in the app is patched; only the camera is scripted.
...
2. **Write the story** ([references/story.md](references/story.md)): one message sentence with its number sources; 3–6 chapters, each titled with the **answer** it gives (not the scene name) and subtitled with the setting; `cards.json` for the intro and summary cards, so the argument exists end to end before any beat; then per chapter 4–15 beats (`scripts/<stem>.json`, format in `python3 scripts/edit_demo.py --help`). A beat is one step of the argument (what happened, who decided, what followed, with the number) or, in a feature tour, what the feature on screen does for the user; ≤ 45 characters, anchored to the second the event is visible. The last beat of a chapter is `"style": "insight"`: the chapter's point in one sentence, which later becomes a row on the summary card. Sentence and voice rules (합니다체, banned symbols, no speed labels): [references/caption-style.md](references/caption-style.md).
...
9. **Check**: read one still per clip at its insight beat and confirm the frame is evidence for that sentence, and that the sentence is a row on the summary card; `ffprobe` duration; one `ffmpeg -ss <t> -frames:v 1` still per clip at a zoomed moment; read the still to confirm a full 1920×1080 frame with no blank margins. If the UI did something other than what the script says (a timeout, a different recovery path), rewrite the beat to what happened; do not keep a sentence the frame contradicts. Before the reel, grep every clip script and `cards.json` for `—`, `·`, `→` and `배속`; any hit is a caption to rewrite.
...
| Zoom on a page rectangle | `rec.region('key', x, y, w, h, { pad: 0, kmax: 1.1 })`; zoom is capped by the rectangle's width, so a 1820 px region reaches only ~1.05×. For readable tables and charts show the left and right halves (~910 px each, `kmax: 1.9`) one after the other |
| Back to wide | `rec.reset('wide')` |
| Click a control | `await tap(page.locator(...))`; Radix Tabs/Select open on `pointerdown`, which `tap()` does not send, so use a real `locator.click()` on those once the element is in view; text inputs: fill them before `rec.install()` or set `.value` and dispatch `input` via `locator.evaluate` |
...
- **Captions that read the screen** ("로봇이 물체를 다시 잡습니다", "System 2 패널이 열립니다", or camera labels like "Robot + decision stack"): the viewer can already see that; they learn nothing. Every beat is a step of the argument, or in a feature tour what the feature does.
- **Captions that sound generated** (user rejected a whole 8-minute reel for this, 2026-09-25): `—`, `·`, `→` joining fragments, "(5배속)" prefixes, "이 녹화에선 누르지 않았습니다", aphorisms like "Registry 는 목록이 아니라 써 보고 고르는 곳이다". Write 합니다체 sentences that say what the feature does; report skipped actions in the chat, not on screen.
- **Measured values in a feature tour** ("12턴인데 Runtime $17.63", "84초, 툴 6번"): the viewer wanted to know what the feature is for, not what this take cost. Keep a number only when it is the feature ("100만 토큰당 가격"). Settings and admin tabs get a beat of their own.
- **Chapters named after scenes** ("Live 낙하", "Lab seed 7") instead of the answer they give ("스텝마다 Laya가 판단합니다"): the reel becomes a tour of pages instead of a story. Title = answer, subtitle = setting.
