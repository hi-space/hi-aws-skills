...
|---|---|---|
| A generic deck-wide entrance build | `-a auto`; with the default `after-previous` Start, groups use fixed `--animation-stagger` timing | Post-processing; §2, §4 |
...
| A transition or object animation needs an audible cue | Optional `transition.sound` or object `sound`, selected only after the visual solution is complete and synced from the global library | Post-motion; §2.2 |
...
```bash
python3 skills/aws-ppt-master/scripts/animation_config.py list-groups <project>   # real anchors after regrouping
python3 skills/aws-ppt-master/scripts/animation_config.py scaffold <project>      # neutral editable scaffold (effect: none, {} placeholders)
python3 skills/aws-ppt-master/scripts/animation_config.py validate <project>      # before export
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py <project>                    # reads <project>/animations.json automatically
```
...
```bash
python3 skills/aws-ppt-master/scripts/sound_sync.py list --query <term>                       # optional exact filtering after the vocabulary is in context
python3 skills/aws-ppt-master/scripts/sound_sync.py <project_path> <namespace>/<sound_id> [...]  # materialize only the chosen ids
```
...
**Hard rule — PPTX and MP4 are separate deliveries**: sound fields and read-back prove the PPTX carries the cue, not that PowerPoint's encoder put it in the MP4. Verify a sound-enabled MP4 by capturing the live Slide Show with system audio, and keep gain/limiter settings out of `animations.json`.
...
```bash
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py <project> -t push --transition-duration 0.6
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py <project> -t none
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py <project> --auto-advance 5            # kiosk playback
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py <project> -t none --auto-advance 5
```
...
| Start | Behavior |
|---|---|
| `on-click` | Each click reveals the next group; only for a controlled semantic reveal |
...
Export rejects an unknown effect, mode, or trigger, invalid timing or order, a missing slide, group, or `trigger_shape`, and any structural-layer target, then reads every slide back and compares it with the request; nothing is downgraded or omitted. The complete rules and the reverse projection `pptx_to_svg.py` writes into `animations.json` are [`pptx-animations.md`](../scripts/docs/pptx-animations.md) §6 and [`pptx-transitions.md`](../scripts/docs/pptx-transitions.md) §6.
...
[`pptx_transitions.py`](../scripts/pptx_transitions.py), [`svg-pipeline.md`](../scripts/docs/svg-pipeline.md), [`pptx-transitions.md`](../scripts/docs/pptx-transitions.md), [`pptx-animations.md`](../scripts/docs/pptx-animations.md).
