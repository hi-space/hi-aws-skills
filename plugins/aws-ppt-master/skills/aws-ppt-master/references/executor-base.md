...
| On-slide text is written or rewritten (any language) | [copy-voice.md](./copy-voice.md) |
...
| Endpoint id | Fix the first endpoint's direct-root `<g id>` when that page is drawn and record the intended partner id with `workflow_log.py` (never in `notes/total.md`, which is embedded verbatim as speaker notes); the next page reuses it, so no endpoint is renamed after both pages exist (a rename reruns the gate) |
...
AWS services are always drawn with `aws/<service>` marks (`aws/lambda`, `aws/s3`, `aws/bedrock`), never with a generic glyph or a recoloured silhouette. Bedrock AgentCore components use `aws/agentcore-<part>-<colour>-<light|dark>`; pick `-dark` on dark page fields.
