...
| `items[].aspect_ratio` | yes | Passed to `image_gen.py --aspect_ratio`; the Bedrock backend accepts a subset of the CLI union (no `3:1`, among others), and `--manifest` fails the item before any request when the resolved backend rejects its ratio |
...
|---|---|---|
| `api`, or `auto` with AWS credentials resolved | **Path A** `image_gen.py --manifest` | One command runs the manifest with concurrency and writes status per item |
...
| Default `host-native` | Path B, skipping A even when AWS credentials resolve |
| Default `manual` | Offline Manual |
| Default `auto` | Path A when AWS credentials resolve (`IMAGE_BACKEND` defaults to `bedrock`; two consecutive failures fall to B), then Path B when the host has a native tool; never Offline Manual by itself |
...
Validates the file behind every `Generated` row before skipping it, iterates retryable rows with bounded concurrency, and writes each status atomically. Interrupting is safe (completed items stay `Generated`), and `--render-md` refreshes the Markdown sidecar after an interruption. Backend selection, `.env` lookup order, and the Bedrock env keys: [`image.md`](../scripts/docs/image.md). The single-image form `image_gen.py "prompt" --filename …` remains for ad-hoc re-rolls. The Bedrock backend returns its own native resolution per aspect ratio: backfill the actual pixels into `Dimensions`, and when a file is far larger than its planned on-slide size, downscale it to that size as a prepared derivative (`image_treat.py --fit WxH`); never upscale.

### Path B — host-native image tool

Automatic when AWS credentials do not resolve or Path A failed and the host (Codex, Antigravity, Claude Code, similar) offers an image tool; the user may also name it explicitly. Prompts come from `items[].prompt`. Never run `image_gen.py --manifest` here, but still run `python3 scripts/image_gen.py --render-md project/images/image_prompts.json` for the sidecar. Batch a few rows at a time (~3–4) when the host runs tools in parallel, serially otherwise. Outputs land at the resource-list filename. Hosts with fixed native resolutions generate at the closest size and backfill the actual pixels into `Dimensions`; never upscale to fake a size (display-side upscaling up to ~1.3× is a non-blocking warning). Mark each item `Generated` as its file lands.
...
**Failure handling** (extends [`image-base.md`](./image-base.md) §3): on `auto`, two consecutive Path A failures fall to Path B without halting; if B also fails, Default enters the decision above and Quick the replan. A confirmed `api` or `host-native` path is retried once, never switched.
