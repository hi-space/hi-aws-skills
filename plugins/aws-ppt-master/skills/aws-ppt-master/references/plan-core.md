...
| `delivery_context` | What is primary — presenter-led, reader-led, hybrid (which leads), or recorded/self-running (no live presenter; timing, transitions, playback)? What secondary use, occasion, and time constraint remain? One open field, never an enum. |
...
**Explicit choice, never a silent default**: reading mode is the user's own answer to *how much of the message lands on the page itself*. Think of it as a detail dial with three stops: 간결/발표용 (`presentation`), 균형 (`balanced`), and 상세/구체 문구까지 (`text`). Default surfaces it as an explicit selection in the confirmation flow, seeded from the contract as a recommendation but shown with its consequence and left for the user to confirm or change, so a deck is never quietly built sparse when the user wanted substance on the page (or the reverse). State the recommended stop and what it will do to the pages, not just the label.

**When the user picks `text` (상세), author the content, not a summary of it**: write the specific sentences, figures, definitions, steps, and qualifiers the slide must actually deliver onto the page (or into a real list item, table, or caption), so a reader with no presenter gets the point itself, not a heading that gestures at it. This is the opposite failure from the padding the copy rules guard against: detail means more *substance* on the page, never more words about the substance. Every line added this way still obeys [`copy-voice.md`](./copy-voice.md): a concrete claim carrying its number, no meta narration, no banned punctuation. A `text` deck that is merely a `presentation` deck with longer sentences has not honored the choice.

**But detail is not a prose dump.** Substance on the page still needs visual hierarchy and a single focal point: lead with the one claim, then break the rest into scannable units (labeled points, a KPI or callout carrying the key number, a small table, an annotated figure) rather than two or three stacked body paragraphs. Pull the number that matters out of the sentence and give it weight (a hero figure, an inline emphasis run, a highlighted row). A wall of body copy is a failure even in `text` mode: the reader should be able to scan the structure and land on the decisive figure. Carry the detail through structure, not through length.
...
**Reference and citation links are source-backed links**: a `sources/*.facts.json` entry may carry a `source_url` (the authoritative reference behind that fact; see [`topic-research.md`](../workflows/stages/topic-research.md)). When a §IX page shows that fact's citation or source on the slide (a source line, a footnote, an "출처" tag, a reference list, or the cited text itself), record the fact's `source_url` as a link on that citation text under the rule above, so the reference is clickable rather than a plain string a reader must retype. This links the citation the reader can already see, never every fact and never a bare URL pasted onto a slide with no citation; use the exact `source_url`, never a guessed or search-result destination, and drop the link (keep the citation) only if that fact has no `source_url`. A page with no visible citation gains no link.
...
|---|---|
| Real subject, place, product, evidence, atmosphere, or scene benefits from visual grounding | Supplied/extracted, AI, or sliced image |
...
**Hard rule — credentials never decide image need**: missing AWS credentials for the Bedrock backend or a host generation credential never justifies `none` or the deletion of a planned `ai` role; do not inspect configuration or probe a provider — acquisition (Default Step 5; Quick §2 resource preparation) is the first capability check. When `ai` is included, preserve an explicit user path instruction, otherwise recommend `auto`.

**Default — visual grounding before `none` (may override when the full-roster review finds no image job)**: honor an explicit no-image requirement; otherwise, when the audience must recognize, experience, compare, or choose an externally verifiable subject, place, product, or setting, plan `provided`, and plan `ai` where invented or stylized expression materially improves a visual job. Mixed sources serve different roles; a rendering candidate resolves how imagery looks, never whether a real subject appears as itself.

**Mandatory — per-image source decision**: outside Image to PPTX, decide each page image's source separately — supplied/extracted when it carries authority, AI when invented or stylized expression matters more than documentary identity; mixed sources are normal. A visual style, `Illus.` propensity, or rendering resolves how imagery looks, never its source: a named place, building, product, artwork, or person stays a supplied candidate however illustrative the deck, and a subject deliberately not shown as itself, or not available as a supplied asset, is stated with its reason in the final report.
...
```bash
python3 skills/aws-ppt-master/scripts/visualization_recall.py validate \
...
**Hard rule — aws-ppt-master default stack**: when neither the user nor a template fixes typography, the stack is `Amazon Ember` for Latin text, and Korean (or any CJK) text pairs `Noto Sans KR` in the same stack — `Amazon Ember, Noto Sans KR` (a serif direction pairs `Noto Serif KR`). If Amazon Ember is not installed or approved on the delivery target, the whole stack falls back to the Noto Sans family — `Noto Sans, Noto Sans KR`. Both families count as approved on the delivery target (export and `pptx_delivery_check.py` raise no font-portability warning for them); never swap them for Windows stock faces (`Segoe UI`, `Malgun Gothic`, `Microsoft YaHei`) unless the user asks. Build the concord/contrast pairing from Amazon Ember weights (Light / Regular / Medium / Bold / Heavy) and size, not from a second Latin family; a user- or template-fixed stack still wins.
