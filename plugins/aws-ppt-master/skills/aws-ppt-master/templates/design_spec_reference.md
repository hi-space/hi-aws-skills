...
| Speaker Notes | <enabled or disabled> — <explicit user instruction, final Stage-2 proactive policy, or workflow default> |
...
With Speaker Notes disabled, §X keeps only `- **Generation**: disabled`. When a final/literal script will become notes, §X `Content` names the source and says `preserve verbatim`, with the segmented script kept in `notes/total.md`.
