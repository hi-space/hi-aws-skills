...
**12,331 SVG icons across six libraries**, embedded directly into generated SVG. Default Strategist or the Quick main agent chooses at most one primary library from the four stylistic ones; the brand-logo libraries (`simple-icons`, `aws`) are prepared as needed for real brands or AWS service marks, alone or alongside it, and are never a separate Confirm UI choice. Upstream versions, licenses, attribution, and trademark boundaries: [THIRD_PARTY_NOTICES.md](./THIRD_PARTY_NOTICES.md).
...
| `aws` | **AWS service marks** — official Architecture Icon Deck style (coloured category square + white glyph) for 248 services, plus 56 Amazon Bedrock AgentCore component variants (`agentcore-<part>-<blue\|cyan\|purple\|teal>-<light\|dark>`), colours fixed | 304 | `0 0 80 80` (AgentCore variants vary) | `aws/` |

`aws` is a brand-mark library like `simple-icons`: it accompanies one stylistic library and never replaces it. Every file carries `data-icon-style="preserve-color"`, so the embedder keeps its own fixed colours and natural (sometimes non-square) viewBox at embed time; `fill` on `<use data-icon="aws/...">` is ignored — do not recolour AWS marks. On dark backgrounds use the `-dark` AgentCore variants instead. For the full official set (800+ icons) copy files from the sibling plugin `aws-diagram-design` (`assets/aws-icons/`) into `<project>/icons/custom/`.
...
```bash
python3 skills/aws-ppt-master/scripts/icon_sync.py <project_path> tabler-outline/home tabler-outline/bulb simple-icons/github
```

Missing names, or one batch mixing the four stylistic libraries, exit non-zero; `simple-icons` and `aws` may coexist. Files under `<project>/icons/` form the prepared pool and combine freely with user-provided, custom, or imported icons; `finalize_svg.py --only embed-icons`, preview, validation, and native export resolve only this pool. **Custom icons**: drop `.svg` files into `<project>/icons/<lib>/` (any `<lib>`, e.g. `custom/`) and reference `data-icon="<lib>/<name>"`. **Imported vectors**: `create-template` reserves `imported/` — one copy at `<workspace>/icons/imported/<name>.svg`, referenced as `data-icon="imported/<name>"`, never duplicated under `templates/` or used as a hand-curated library.
...
<use data-icon="aws/lambda" x="100" y="200" width="48" height="48"/>
```
...
```bash
rg --files "skills/aws-ppt-master/templates/icons/tabler-outline" -g '*chart*.svg'
rg --files "skills/aws-ppt-master/templates/icons/simple-icons" -g '*github*.svg'
```
