...
# AWS PPT Master Skill

AWS PPT Master is a routed presentation workflow. This entry owns global execution discipline and route selection only; each selected route owns its procedure.
...
`${SKILL_DIR}` and replace any `skills/aws-ppt-master/` prefix with it. Never `cd`,
...
- AI image generation uses Amazon Bedrock only (`scripts/image_gen.py`, backend `bedrock`). Configure AWS credentials and `AWS_REGION` before Path A; see `scripts/docs/image.md`.
- Default typography is `Amazon Ember` (Latin) paired with `Noto Sans KR` (Korean/CJK); when Amazon Ember is not available on the delivery target, the stack falls back to `Noto Sans, Noto Sans KR`. A user- or template-fixed stack always wins. Owner: `references/plan-core.md` §6.2.
