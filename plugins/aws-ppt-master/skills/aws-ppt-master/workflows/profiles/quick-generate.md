...
**Default — optional production behavior (may override when useful)**: Speaker Notes and Custom Animations start off. Enable either when the request or deck benefits, with their normal inputs and flags and without asking. Quick never creates or reads a root Design Spec or lock to do so.
...
- **Production outcomes**: effective Speaker Notes and Custom Animations.
...
**Prepared final script**: an explicit final/literal script for notes is segmented by scene while resolving the roster, every word preserved, and written once before P01 to `notes/total.md` (`# Slide <number>` headings, `---` separators) as production input, split only after the roster exists. Draft notes stay source material for the ordinary notes branch.
...
**Validation before §3**: every file-backed resource is terminal — `Existing` or `Generated` under [`svg-image-embedding.md`](../../references/svg-image-embedding.md) — and every `slice_names` basename resolves to its PNG; a missing name resumes its owning step and is never deferred to the checker. Never bypass status by preview or presence, never substitute unrelated material. A row stuck at `Needs-Manual` blocks even with an unverified file.

**Inspection boundary**: acquisition-time review follows the owning reference; authoring inspects only one ambiguous `Existing`/`Generated` asset under `executor-image.md` and never reopens `Generated` outputs (Image to PPTX inspects every normalized page and generated layer once, then the final recomposition). After resources change, run `analyze_images.py`; manifests and provenance are resource truth, not a design strategy.
...
**Notes** (when enabled): load [`executor-notes.md`](../../references/executor-notes.md) after the passing check, validate a frozen script or pre-SVG notes draft without regenerating it or otherwise generate `notes/total.md` from the final roster, then split:
...
**Motion and sound**: run [`customize-animations`](../stages/customize-animations.md) after the notes pass when the §1 outcome or an existing sidecar triggers it; deck-wide-only motion uses exporter flags. After motion is final, run the optional sound pass in [`animations.md`](../../references/animations.md) §2.2.
...
- [ ] **Next**: report the base PPTX, plus the resolved mode, visual style, and image sources actually used; for every no-AI replan, report the affected job, attempted path, concrete error, replacement carrier, and that retaining AI imagery requires repairing generation capability and a new Quick run
```
