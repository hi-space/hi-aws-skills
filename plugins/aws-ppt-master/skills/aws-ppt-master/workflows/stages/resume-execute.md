...
Read `skills/aws-ppt-master/workflows/generate-pptx.md` and jump to `### Step 6: Executor Phase`, which loads `executor-base.md` and applies its context policy: read the complete Design Spec, then the complete lock, once; resolve the effective Speaker Notes / Custom Animations outcomes from `design_spec.md §I` (missing outcomes default `enabled` / `disabled`; never from the lock).
...
|---|---|---|
| `notes/total.md` | §X records a supplied final/literal script | Return to Step 4 and repair the Design Spec so §X records the script; never rewrite the script from memory |
| `images/` plus files whose row status requires existence (`Existing` / `Generated`; an absent `Needs-Manual` file is allowed until the Step 7 gate) | `spec_lock images` references any image | By provenance: `Acquire Via: user` / `Existing` is a required manual artifact (`failure-recovery.md` §2, wait for the exact file); a template-bundled bitmap returns to Step 3 to restore the workspace; AI or slice output uses its `failure-recovery.md` §1 row. Formula markers never create a required image file |
