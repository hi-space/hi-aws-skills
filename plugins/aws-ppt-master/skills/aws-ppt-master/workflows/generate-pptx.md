...
**Production fields**: resolve Speaker Notes and Custom Animations as latest explicit user instruction → final Stage-2 proactive value → default `true` / `false`. Persist the effective outcomes with provenance as the two rows in `design_spec.md §I`, keep the raw proactive fields as evidence only, and project neither into `spec_lock.md`. A later explicit request updates only its §I outcome and resumes the owning step without reopening Confirm UI.
...
**Output**: `design_spec.md` and `spec_lock.md`.
...
**Trigger**: §VIII has at least one `Acquire Via: ai` or `slice` row, or a pending derivative declared by `Reference: Derived from <canonical bare filename>; treatment=...`. Prepared-user-only plans and `placeholder` rows do not trigger it; a permitted but unused source creates no row. If §VIII omits a source, asset, or page role that `image_notes` explicitly requires, return to Step 4 Gate 1 and repair from the retained final state.
...
Load only the references the rows need; a deck with prepared derivatives may write both `image_prompts.json` and `image_sources.json`. The positional `image_gen.py "prompt"` form is for out-of-pipeline fixups and the §4.4 reconstruction derivation only.
...
Sound: the optional post-motion pass is [`animations.md`](../references/animations.md) §2.2.
...
- [ ] **Next**: report the exported PPTX path; run a supporting post-export stage only on its explicit trigger
