...
**Proactive production resolution**: keep final Stage-2 raw fields as evidence; resolve durable outcomes as explicit instruction → final Stage 2 → workflow defaults `enabled` / `disabled`. Record outcomes/provenance only in Design Spec §I, never the lock.
