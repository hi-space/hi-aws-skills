...
```bash
python3 skills/aws-ppt-master/scripts/pptx_to_svg.py source.pptx \
  -o /path/to/workspace --inheritance-mode both --roundtrip
python3 skills/aws-ppt-master/scripts/svg_to_pptx.py /path/to/workspace \
...
scripts = Path("skills/aws-ppt-master/scripts")
...
```bash
python3 -c "import sys; sys.path.insert(0, 'skills/aws-ppt-master/scripts'); from svg_to_pptx.native_objects.formula_compiler import compile_latex_to_omml as c; c(sys.argv[1])" '\frac{a}{b} \int_0^T e^{-i\omega t}\, dt'
```
...
- On every SVG-authoring route, each file in `svg_output/` is the complete visible
  ...
  transitions, and direct native-PPTX workflows keep their separate
...
- Recorded narration is opt-in and expects user-supplied audio, one file per slide, under `audio/`:
