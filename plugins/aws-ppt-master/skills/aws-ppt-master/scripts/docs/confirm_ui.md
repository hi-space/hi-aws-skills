...
- **Proactive execution booleans** — Final Stage 2 carries top-level `proactive_speaker_notes` and `proactive_custom_animations` values. Defaults are `true` and `false`, respectively. They control what the Agent does proactively only when the user has not explicitly instructed otherwise; the latest explicit user instruction always wins. These two values are raw confirmation evidence: the UI and server neither couple nor rewrite them, and every boolean combination is valid. Disabling proactive custom animation does not suppress the Strategist's advisory motion recommendations.
...
`image_ai_path` is conditional: the page shows it and writes it to `result.json` only when `image_usage` includes `ai`. User-provided / Placeholder / No images paths do not carry an AI backend choice.
...
- The two proactive-execution fields are top-level boolean `{ "value": ... }` objects, not catalog ids. Omitted fields use `true / false` for notes / custom animation. These are absence-of-instruction defaults, not permission to override the user's latest explicit request. Preserve both raw values independently through `result.json`; do not couple or rewrite them. `proactive_custom_animations: false` leaves Strategist animation suggestions unchanged; it only prevents unrequested custom-animation execution.
...
that omit them resolve to `true / false`. They remain raw evidence; Strategist
owns the effective dependency resolution described above.
