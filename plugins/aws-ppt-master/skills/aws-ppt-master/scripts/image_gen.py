...
Backend: bedrock (Amazon Bedrock, Stability AI Stable Image Core / Ultra / SD3.5 Large).
...
Supported keys:
  IMAGE_BACKEND        optional, default bedrock (the only valid value)

  Bedrock-specific keys:
    AWS_PROFILE / AWS_REGION            credentials + region (boto3 default chain)
    BEDROCK_IMAGE_MODEL                 default stability.stable-image-core-v1:1
    BEDROCK_NEGATIVE_PROMPT / BEDROCK_SEED
...
IMAGE_ENV_PREFIXES = ("IMAGE_", "AWS_", "BEDROCK_")
...
def _load_image_env_file() -> Path | None:
    ...
    """
    replacements = {
        "IMAGE_API_KEY": "BEDROCK_IMAGE_MODEL / AWS_REGION",
        "IMAGE_MODEL": "BEDROCK_IMAGE_MODEL / AWS_REGION",
        "IMAGE_BASE_URL": "BEDROCK_IMAGE_MODEL / AWS_REGION",
    }
    deprecated_messages = {
        key: (
...
def _resolve_backend() -> tuple[object, str]:
    ...
    """
    backend_name = os.environ.get("IMAGE_BACKEND", "bedrock").strip().lower() or "bedrock"
    canonical = BACKEND_ALIASES.get(backend_name)
    if not canonical:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "that support editing accept this (the bedrock backend does not). Not "
    ...
    if args.reference_image is not None:
        if not getattr(backend, "SUPPORTS_REFERENCE_IMAGE", False):
            print(
                ...
                "(--reference-image). No currently supported backend does."
            )
            sys.exit(1)
