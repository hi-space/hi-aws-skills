...
PPT_SAFE_FONTS = frozenset({
    ...
    'impact',
    # aws-ppt-master default typography: Amazon Ember first, Noto Sans as
    # the fallback family (Noto Sans KR / Noto Serif KR for Korean text).
    # Treated as approved on the delivery target, so export and the
    # delivery check do not raise a font-portability warning for them.
    'amazon ember', 'amazon ember display', 'amazon ember mono',
    'noto sans', 'noto sans kr', 'noto serif kr',
})
...
_EA_DEFAULTS_BY_LANGUAGE = (
    ...
    # aws-ppt-master: Korean text pairs with the Noto Sans family (the
    # fork's fallback typography), not the Windows stock Malgun Gothic.
