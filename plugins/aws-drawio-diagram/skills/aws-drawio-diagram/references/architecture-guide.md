...
picture: it walks the architecture **one numbered step per relationship**, in sentences, and each step number is
the **badge drawn on that edge** — the reader finds ③ on the picture and reads step 3. Write it after the
Reviewer's `ready`, from the brief and the final `.drawio.png`. It is the third deliverable next to `.drawio` and
`.drawio.png`, and `scripts/check_guide.py <name>.guide.md <name>.brief.md` must pass before hand-off.
...
**Numbering.** One step per row of the brief's Relationships, in `#` order, list number = `#` = badge. A bent edge
carries only its badge and no text — its step is where the reader learns what flows. An aux row that is not drawn
...
- **선** — 실선 = 동기 호출, 점선 = 비동기·보조, 빨간 점선 = 오류 경로. 선 위의 **번호 배지**가 아래 단계 번호이고,
  글자 라벨은 자리가 있는 선에만 있다. 글자가 없는 선은 그 번호의 단계에서 설명한다.
- 사용자와 외부 시스템은 AWS Cloud 상자 밖에 있다.

## 단계별 흐름
<관계표 한 행에 한 단계, `#` 순서, 목록 번호 = `#` = 배지 번호. 각 단계: **From → To**를 굵게, 이어서 무엇이
흐르고 왜인지, 동기/비동기, 실패하면 어떻게 되는지(중요할 때). 서비스 이름은 브리프의 표기 그대로(영문).>

1. **Mobile client → API Gateway** — 앱이 REST 엔드포인트를 HTTPS로 호출한다. API Gateway가 TLS를 종료하고
   요청 형식을 검증하고 스로틀링한다. (라벨 `HTTPS`)
...
3. **API Gateway → Cognito** — 모든 호출에서 bearer 토큰을 검증한다. 점선: 요청 데이터 경로의 한 홉이 아니라
   요청 옆에서 도는 검사다. (라벨 `verify JWT`)
4. …
7. **Step Functions → Payment** — 결제 Lambda를 task로 호출한다. 선이 꺾여서 배지만 있다; R8 Inventory 호출과
   같은 종류의 task 호출이다.
…
12. **DynamoDB → S3** — 주문 테이블을 주기적으로 S3에 내보낸다(point-in-time export). 점선·보조. (라벨 `export`)
...
- **One step per relationship, numbered like the brief's `#`** — primary and aux, drawn or not. The checker refuses
  a guide with a missing step number or a step that does not name both endpoints (component id or the brief's
  service name). Re-numbering the brief after Phase 3 breaks the badges: do not.
...
- Name the text label when the edge has one ("(라벨 `HTTPS`)") and say "배지만" / "badge only" when it bends, so a
  reader knows what to look for on the picture.
