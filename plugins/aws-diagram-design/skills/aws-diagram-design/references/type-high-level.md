...
Fills alternate `#232F3E` / `#3d4460` (light mode) or `#3d4460` / `#4a5270` (dark mode). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, `text-anchor=middle`, centered at `chevron_cx, 21`.
...
Stroke: `rgba(35,47,62,0.20)`, `stroke-width=0.8`, `stroke-dasharray=6,3`, `rx=6`. Zone fill: `rgba(35,47,62,0.02)`.
...
Stroke: `rgba(35,47,62,0.18)`, `stroke-width=1.2`, `rx=8`. Fill: `rgba(35,47,62,0.02)`. K8s icon + label at `(cluster_x + 16, 352)` (icon) and `(cluster_x + 40, 362)` (text).
...
Stroke: `rgba(35,47,62,0.20)`, `stroke-width=0.8`, `rx=6`. Fill: `rgba(35,47,62,0.05)`. Icon at `(16, cross_y(k) + 10)`, name centered at `(effective_w / 2, cross_y(k) + 22)`, subtitle at `(effective_w / 2, cross_y(k) + 34)`.
...
Stroke: `rgba(35,47,62,0.18)`, `stroke-width=0.8`, `rx=4`. Fill: `rgba(35,47,62,0.05)`. Tool icon at the far right (`bar_x + bar_w - 50, 58`); name centered at `(bar_x + bar_w/2, 71)`; subtitle at `(bar_x + bar_w/2, 84)`.
...
**Focal node:** `fill="rgba(255,153,0,0.08)"`, `stroke="#EC7211"`, `stroke-width=1.2`. Title text in accent color. All other nodes: white fill, `stroke=rgba(35,47,62,0.25)`, `stroke-width=1`.
...
Fills alternate `#232F3E` / `#3d4460` (same palette as horizontals). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, **rotated −90°**, anchored at `(strip_x + strip_w/2, (y0+y1)/2)`.
...
|---|---|---|---|---|---|
| `primary` | `#EC7211` | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the `focal` node. |
| `secondary` | `#545B64` | 1.0 | — | `arrow` | Default for source→component and component→component when neither endpoint is focal. |
| `trigger` | `#545B64` | 1.0 | `4,3` | `arrow-sm` | Every edge originating from a `kind: bar` component. |
| `query` | `rgba(35,47,62,0.30)` | 1.0 | `4,3` | `arrow` | Read-back edges (e.g., focal ↔ Trino). |

**Defs block** (required, exactly these four markers):

```svg
<defs>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#545B64"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#EC7211"/></marker>
  <marker id="arrow-sm"     markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto"><polygon points="0 0, 6 2.5, 0 5" fill="#545B64"/></marker>
  <marker id="arrow-dim"    markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="rgba(35,47,62,0.45)"/></marker>
</defs>
```
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#ffffff` | `#1c1f2e` |
| Ink | `#232F3E` | `#ffffff` |
| Muted text | `#545B64` | `rgba(255,255,255,0.65)` |
| Chevron dark fill | `#232F3E` | `#3d4460` |
| Chevron light fill | `#3d4460` | `#4a5270` |
| Chevron label | `#ffffff` | `#ffffff` (unchanged) |
| Dashed border | `rgba(35,47,62,0.20)` | `rgba(255,255,255,0.22)` |
| Cluster border | `rgba(35,47,62,0.18)` | `rgba(255,255,255,0.18)` |
| Node fill | white | `rgba(255,255,255,0.06)` |
| Node stroke | `rgba(35,47,62,0.25)` | `rgba(255,255,255,0.20)` |
| Focal fill | `rgba(255,153,0,0.08)` | `rgba(255,153,0,0.12)` |
| Focal stroke | `#EC7211` | `#FF9900` |
| Accent connector | `#EC7211` | `#FF9900` |
| Dot pattern | `rgba(35,47,62,0.10)` | `rgba(255,255,255,0.10)` |
